...
impl From<SocksProxyError> for ResponseCode {
    fn from(e: SocksProxyError) -> Self {
        match e {
            SocksProxyError::Socks(e) => e,
            SocksProxyError::Io(_) => ResponseCode::Failure,
        }
    }
}
    ...
    pub async fn serve(&mut self) {
        ...
        while let Ok((stream, _client_addr)) = self.listener.accept().await {
            ...
            tokio::spawn(async move {
                let mut client = SOCKClient::new(stream, auth_methods, timeout);
                client.init().await
            });
        }
    }
    ...
    T: AsyncRead + AsyncWrite + Send + Unpin + 'static,
{
    ...
    pub fn new(stream: T, auth_methods: Arc<Vec<u8>>, timeout: Option<Duration>) -> Self {
        SOCKClient {
            stream,
            auth_nmethods: 0,
            socks_version: 0,
    ...
    async fn auth(&mut self) -> Result<(), SocksProxyError> {
        ...
        if methods.contains(&(AuthMethods::UserPass as u8)) {
            ...
            trace!("Sending USER/PASS packet");
            ...
            trace!("Auth Header: [{}, {}]", header[0], header[1]);
            ...
            Err(SocksProxyError::Socks(ResponseCode::Failure))
        }
    }

    /// Handles a client
    pub async fn handle_client(&mut self) -> Result<usize, SocksProxyError> {
        trace!("Starting to relay data");
        ...
        debug!("Connecting to {}", displayed_addr);
        ...
        match req.command {
            ...
            SockCommand::Connect => {
                ...
                let mut target =
                    ...
                    .await
                    .map_err(|_| SocksProxyError::Socks(ResponseCode::ConnectionRefused))??;
                ...
                match tokio::io::copy_bidirectional(&mut self.stream, &mut target).await {
                    ...
                    Err(e) => Err(SocksProxyError::Io(e)),
                    Ok((_s_to_t, t_to_s)) => Ok(t_to_s as usize),
                }
            }
            SockCommand::Bind => Err(SocksProxyError::Io(std::io::Error::new(
                std::io::ErrorKind::Unsupported,
                "Bind not supported",
            ))),
            SockCommand::UdpAssosiate => Err(SocksProxyError::Io(std::io::Error::new(
                std::io::ErrorKind::Unsupported,
...
impl SOCKSReq {
    ...
    async fn from_stream<T>(stream: &mut T) -> Result<Self, SocksProxyError>
    where
        T: AsyncRead + AsyncWrite + Send + Unpin + 'static,
    {
        ...
        let command = match SockCommand::from(packet[1] as usize) {
            ...
            None => {
                ...
                stream.shutdown().await?;
                Err(SocksProxyError::Socks(ResponseCode::CommandNotSupported))
            }
        }?;
        ...
        let addr_type = match AddrType::from(packet[3] as usize) {
            ...
            None => {
                error!("No Addr");
                stream.shutdown().await?;
                Err(SocksProxyError::Socks(ResponseCode::AddrTypeNotSupported))
            }
        }?;
